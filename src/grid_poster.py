...
class GridPoster(poster.Poster):
    def __init__(self):
        poster.Poster.__init__(self)
    ...
    def __draw_tracks(self, d, w, h, offset_x, offset_y):
        print("GridPoster::__draw_tracks")
        size, (count_x, count_y) = self.__compute_grid(len(self.tracks), w, h)
        ...
        for (index, track) in enumerate(self.tracks):
            x = index % count_x
            y = index // count_x
            color = self.colors['highlight'] if track.highlight else self.colors['track']
